...
if __name__ == '__main__':
    demand = Demand(1, "0", "1", 78.3231)
    demand.path_lenght = 700
    demand.actual_bitrate = 700
    print([x.name for x in demand.possible_modulations])
    print([x.max_lenght for x in demand.possible_modulations])

    demand.select_modulation_with_min_transceivers()
    # for modulation in demand.possible_modulations:
    #     demand.select_modulation(modulation)
    #     print(f"Transceiver {demand.number_of_transceivers}")
    #     print(f"Slots {demand.required_slots}")
